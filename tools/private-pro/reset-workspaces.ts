...
export function buildPrivateProResetPlan(input: PrivateProResetPlanInput): PrivateProResetPlan {
  ...
  const actions = uids.map(uid => {
    ...
    const accountEpoch = validNonnegativeInteger(account?.data?.accessEpoch);
    const claimEpoch = validNonnegativeInteger(identity.claims.privateProEpoch);
    const accessEpoch = Math.max(accountEpoch, claimEpoch, 0) + 1;
    return {
      uid,
      approved: true,
      ...targets,
      ...
      epochTransition: { from: Math.max(accountEpoch, claimEpoch, 0), to: accessEpoch },
    };
  });
...
async function executeAction(
  ...
  auth: ReturnType<AdminModules['getAuth']>,
  ...
  for (const prefix of action.storagePrefixes) await deleteStoragePrefix(bucket, prefix);
...
async function main() {
  ...
  for (const action of plan.actions) {
    const counts = await inspectAction(action, inputs.firestore, inputs.bucket, admin.FieldPath.documentId());
    accounts.push(counts);
    if (plan.mode === 'execute') await executeAction(action, inputs.auth, inputs.firestore, inputs.bucket);
  }
  console.log(JSON.stringify({
    projectId: plan.projectId,
    mode: plan.mode,
    authIdentityCount: plan.authIdentityUids.length,
    authDeleteCount: plan.authDeleteCount,
    accountCount: plan.actions.length,
    accounts,
  }, null, 2));
}
