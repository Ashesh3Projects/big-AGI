...
export const PRIVATE_PRO_SYNC_SETTINGS_CALL_ID = 'call';
export const PRIVATE_PRO_SYNC_SETTINGS_CHAT_ID = 'chat';
export const PRIVATE_PRO_SYNC_SETTINGS_PURPOSES_ID = 'purposes';
export const PRIVATE_PRO_SYNC_SETTINGS_AI_ID = 'ai-preferences';
export const PRIVATE_PRO_SYNC_SETTINGS_UI_ID = 'ui';
export const PRIVATE_PRO_SYNC_SETTINGS_UX_LABS_ID = 'ux-labs';
export const PRIVATE_PRO_SYNC_SETTINGS_BEAM_ID = 'beam';
export const PRIVATE_PRO_SYNC_SETTINGS_BROWSING_ID = 'browsing';
export const PRIVATE_PRO_SYNC_SETTINGS_IMAGE_ID = 'image';
export const PRIVATE_PRO_SYNC_SETTINGS_FOLDERS_ID = 'folders';

const CallSettingsSchema = z.object({
  group: z.literal(PRIVATE_PRO_SYNC_SETTINGS_CALL_ID),
  grayUI: z.boolean(),
...
const ChatSettingsSchema = z.object({
  group: z.literal(PRIVATE_PRO_SYNC_SETTINGS_CHAT_ID),
...
const UXLabsSettingsSchema = z.object({
  group: z.literal(PRIVATE_PRO_SYNC_SETTINGS_UX_LABS_ID),
...
const BeamSettingsSchema = z.object({
  group: z.literal(PRIVATE_PRO_SYNC_SETTINGS_BEAM_ID),
...
const BrowsingSettingsSchema = z.object({
  group: z.literal(PRIVATE_PRO_SYNC_SETTINGS_BROWSING_ID),
...
const ImageSettingsSchema = z.object({
  group: z.literal(PRIVATE_PRO_SYNC_SETTINGS_IMAGE_ID),
...
const FoldersSettingsSchema = z.object({
  group: z.literal(PRIVATE_PRO_SYNC_SETTINGS_FOLDERS_ID),
...
export function privateProPortableAppSettingsSnapshot(): PrivateProPortableAppSettings[] {
  return PrivateProPortableAppSettingsSchema.array().parse([
    { group: PRIVATE_PRO_SYNC_SETTINGS_CALL_ID, ...callState(useAppCallStore.getState()) },
    { group: PRIVATE_PRO_SYNC_SETTINGS_CHAT_ID, ...chatState(useAppChatStore.getState()) },
    { group: PRIVATE_PRO_SYNC_SETTINGS_PURPOSES_ID, ...purposesState(usePurposeStore.getState()) },
    { group: PRIVATE_PRO_SYNC_SETTINGS_AI_ID, ...aiState(useAIPreferencesStore.getState()) },
    { group: PRIVATE_PRO_SYNC_SETTINGS_UI_ID, ...uiState(useUIPreferencesStore.getState()) },
    { group: PRIVATE_PRO_SYNC_SETTINGS_UX_LABS_ID, ...uxLabsState(useUXLabsStore.getState()) },
    { group: PRIVATE_PRO_SYNC_SETTINGS_BEAM_ID, ...beamState(useModuleBeamStore.getState()) },
    { group: PRIVATE_PRO_SYNC_SETTINGS_BROWSING_ID, ...browsingState(useBrowseStore.getState()) },
    { group: PRIVATE_PRO_SYNC_SETTINGS_IMAGE_ID, ...imageState(useT2IStore.getState()) },
    { group: PRIVATE_PRO_SYNC_SETTINGS_FOLDERS_ID, ...foldersState(useFolderStore.getState()) },
  ]);
}
...
export function privateProPortableAppSettingsApply(value: PrivateProPortableAppSettings): void {
  ...
  switch (parsed.group) {
    case PRIVATE_PRO_SYNC_SETTINGS_CALL_ID: {
      const { group: _group, ...state } = parsed;
      useAppCallStore.setState(state);
      return;
    }
    case PRIVATE_PRO_SYNC_SETTINGS_CHAT_ID: {
      const { group: _group, ...state } = parsed;
      useAppChatStore.setState(state);
      return;
    }
    case PRIVATE_PRO_SYNC_SETTINGS_PURPOSES_ID: {
      const { group: _group, ...state } = parsed;
      usePurposeStore.setState(state);
      return;
    }
    case PRIVATE_PRO_SYNC_SETTINGS_AI_ID: {
      const { group: _group, ...state } = parsed;
      useAIPreferencesStore.setState(state);
      return;
    }
    case PRIVATE_PRO_SYNC_SETTINGS_UI_ID: {
      const { group: _group, ...state } = parsed;
      useUIPreferencesStore.setState(state);
      return;
    }
    case PRIVATE_PRO_SYNC_SETTINGS_UX_LABS_ID: {
      const { group: _group, ...state } = parsed;
      useUXLabsStore.setState(state);
      return;
    }
    case PRIVATE_PRO_SYNC_SETTINGS_BEAM_ID: {
      const { group: _group, ...state } = parsed;
      useModuleBeamStore.setState(state);
      return;
    }
    case PRIVATE_PRO_SYNC_SETTINGS_BROWSING_ID: {
      const { group: _group, ...state } = parsed;
      useBrowseStore.setState(state);
      return;
    }
    case PRIVATE_PRO_SYNC_SETTINGS_IMAGE_ID: {
      const { group: _group, ...state } = parsed;
      useT2IStore.setState({ ...state, hasInitializedLlms: true });
      return;
    }
    case PRIVATE_PRO_SYNC_SETTINGS_FOLDERS_ID: {
      const { group: _group, ...state } = parsed;
...
export function privateProPortableAppSettingsReset(group: PrivateProPortableAppSettings['group']): void {
  switch (group) {
    case PRIVATE_PRO_SYNC_SETTINGS_CALL_ID: useAppCallStore.setState(callState(useAppCallStore.getInitialState())); return;
    case PRIVATE_PRO_SYNC_SETTINGS_CHAT_ID: useAppChatStore.setState(chatState(useAppChatStore.getInitialState())); return;
    case PRIVATE_PRO_SYNC_SETTINGS_PURPOSES_ID: usePurposeStore.setState(purposesState(usePurposeStore.getInitialState())); return;
    case PRIVATE_PRO_SYNC_SETTINGS_AI_ID: useAIPreferencesStore.setState(aiState(useAIPreferencesStore.getInitialState())); return;
    case PRIVATE_PRO_SYNC_SETTINGS_UI_ID: useUIPreferencesStore.setState(uiState(useUIPreferencesStore.getInitialState())); return;
    case PRIVATE_PRO_SYNC_SETTINGS_UX_LABS_ID: useUXLabsStore.setState(uxLabsState(useUXLabsStore.getInitialState())); return;
    case PRIVATE_PRO_SYNC_SETTINGS_BEAM_ID: useModuleBeamStore.setState(beamState(useModuleBeamStore.getInitialState())); return;
    case PRIVATE_PRO_SYNC_SETTINGS_BROWSING_ID: useBrowseStore.setState(browsingState(useBrowseStore.getInitialState())); return;
    case PRIVATE_PRO_SYNC_SETTINGS_IMAGE_ID: useT2IStore.setState({ ...imageState(useT2IStore.getInitialState()), hasInitializedLlms: false }); return;
    case PRIVATE_PRO_SYNC_SETTINGS_FOLDERS_ID: useFolderStore.setState(foldersState(useFolderStore.getInitialState())); return;
  }
}
