...
The audit gets the release commit from `git rev-parse HEAD` and requires `git status --porcelain --untracked-files=normal` to be empty. A dirty checkout blocks even when HEAD matches. `PRIVATE_PRO_RESTORE_EVIDENCE_EXPECTED_COMMIT_SHA` is optional and can only add another equality constraint; it cannot replace actual HEAD. Missing/unconfigured trust, unsigned evidence, wrong signer/key ID/issuer/CI claims, invalid signature, or a signed field change blocks. Valid evidence completion and artifact times must be no older than 90 days, attestation must be within 24 hours after completion, and timestamps more than five minutes in the future block. The canonical evidence file does not exist because no restore rehearsal has run.
...
The evidence file is generated by rehearsal tooling or an operator collection pipeline, then sent as an immutable artifact to an independent CI/release attestor. Do not create it by editing pass/fail booleans. The attestor must fetch immutable artifacts, verify or re-run the required measurements, confirm the exact release commit and protected CI claims, sort JSON object keys recursively, exclude only `signatureBase64`, sign the UTF-8 canonical JSON with its unavailable-to-author Ed25519 private key, then add `signatureBase64`. The audit independently repeats canonicalization and verifies against the pinned public key/trust policy.
