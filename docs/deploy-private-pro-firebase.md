...
No restore rehearsal may target `(default)`. The audit requires fresh independently attested evidence at the canonical absolute `infra/private-pro/firestore-restore-evidence.json` path, or an absolute override whose real path is under `PRIVATE_PRO_RESTORE_EVIDENCE_ROOT`. Trust is pinned by `infra/private-pro/firestore-restore-attestor-trust.json`, which remains intentionally unconfigured until an approval-gated independent CI/KMS Ed25519 attestor is installed. Actual release identity comes from `git rev-parse HEAD`, and the audited checkout must be clean; an optional expected-commit environment value can only further constrain it. Missing/unconfigured trust, dirty checkout, invalid signature, wrong key/issuer/workflow/repository/ref/protected environment/run/release, stale or reversed time, source fingerprint change, unsafe/symlink path, malformed/duplicate/oversized JSON, default/non-empty target, manifest/count/keyed-hash/application/cleanup/attestation failure blocks production release. The rehearsal author/operator must not hold the attestor private key. Delete rehearsal resources only with separate cleanup approval.
