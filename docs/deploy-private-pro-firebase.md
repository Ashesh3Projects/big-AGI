...
No restore rehearsal may target `(default)`. The audit requires fresh provenance-bound evidence at the canonical absolute `infra/private-pro/firestore-restore-evidence.json` path, or an absolute override whose real path is under `PRIVATE_PRO_RESTORE_EVIDENCE_ROOT`. It also requires `PRIVATE_PRO_RESTORE_EVIDENCE_EXPECTED_COMMIT_SHA` and the audit-time-only `PRIVATE_PRO_RESTORE_EVIDENCE_HMAC_KEY`. Missing/invalid MAC, wrong release commit, stale or reversed time, source fingerprint change, unsafe path, malformed/duplicate/oversized JSON, default/non-empty target, manifest/count/keyed-hash/application/cleanup/attestation failure blocks production release. Evidence is generated and MACed by rehearsal tooling or an operator collection pipeline, not handwritten as booleans. Delete rehearsal resources only with separate cleanup approval.
