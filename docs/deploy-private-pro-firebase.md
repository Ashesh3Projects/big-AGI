...
No restore rehearsal may target `(default)`. The audit requires fresh independently attested evidence only through `PRIVATE_PRO_FIRESTORE_RESTORE_EVIDENCE_BASE64`, containing canonical base64 of the externally stored signed canonical JSON artifact. Decoded evidence is capped at 16 KiB; malformed/noncanonical base64, UTF-8 BOM, invalid UTF-8, noncanonical or duplicate-key JSON, and oversized input block. At audit startup, the encoded evidence and optional expected commit are copied into a frozen local input, then active and obsolete restore-evidence and restore-HMAC variables are deleted from `process.env` before collectors, GoogleAuth, or credential executables can start. Google ADC and WIF variables remain available. The audit never restores consumed evidence variables and passes the captured input explicitly to the verifier. Trust is loaded from the exact audited HEAD blob at `infra/private-pro/firestore-restore-attestor-trust.json`, not from a mutable working-tree file, and remains intentionally unconfigured until an approval-gated independent CI/KMS Ed25519 attestor is installed. Attestation, signing, private-key, and access-token values are also scrubbed from the generic child-process wrapper. Actual release identity comes from `git rev-parse HEAD`, and the audited checkout must be clean; an optional expected-commit environment value can only further constrain it. Missing/unconfigured trust, dirty checkout, invalid signature, wrong key/issuer/workflow/repository/ref/protected environment/run/release, stale or reversed time, source fingerprint change, default/non-empty target, manifest/count/keyed-hash/application/cleanup/attestation failure blocks production release. The rehearsal author/operator must not hold the attestor private key. Delete rehearsal resources only with separate cleanup approval.
