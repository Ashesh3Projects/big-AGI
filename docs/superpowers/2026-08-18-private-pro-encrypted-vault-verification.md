...
- The client logger accepts arbitrary message/details and has no general secret-sanitization guarantee. It is therefore treated as sensitive-device storage, never durable in Private Pro, and analytics reporting remains disabled.
...
- Backup import uses a recoverable restore session. Each Firestore transaction is bounded to 200 records and 4 MiB, while the authenticated backup and restore manifest are capped at 100,000 records, 500 chunks, and 128 MiB. The active receipt stores only bounded counts, per-chunk canonical SHA-256 fingerprints, total bytes, progress, and opaque fingerprints. The server rejects count, sum, order, content, byte-total, and overcommit mismatches before writes.
- After all chunks commit, `sealBackupRestore` changes the active receipt to `awaiting-verification` without clearing it. Normal index reads and writes remain blocked. Session-authorized reads fetch the exact index and record set; the client verifies canonical values and hydrates the runtime and durable cache from that snapshot. Only `confirmBackupRestoreVerified` creates the completion receipt and deletes the active marker transactionally. Verification or hydration failure leaves the sealed marker for restart recovery. Seal and confirmation are exact-operation idempotent, including an empty backup with zero chunks.
- Ambiguous tRPC failures are wrapped as `PrivateProVaultAmbiguousTransportError` and replay the exact same operation ID once while online. The classifier covers network failures plus actual `httpLink` empty, truncated JSON, and HTML/non-JSON response parsing failures. Structured tRPC server errors and aborts are never classified as ambiguous.
